...
class DetectionList(list):
    def get_values(self, key, idx: int | None = None):
        if idx is None:
            return [detection[key] for detection in self]
        else:
            return [detection[key][idx] for detection in self]

    def get_stacked_values_torch(self, key, idx: int | None = None):
        values = []
    ...
    def get_stacked_values_numpy(self, key, idx: int | None = None):
