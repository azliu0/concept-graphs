...
def merge_obj2_into_obj1(
    ...
    obj1["text_ft"] = (obj1["text_ft"] * n_obj1_det + obj2["text_ft"] * n_obj2_det) / (
        n_obj1_det + n_obj2_det
    )
...
def compute_overlap_matrix_general(
    ...
    print("about to compute pairwise overlaps", len_a, len_b)
    # Compute the pairwise overlaps
    for idx_a in tqdm(range(len_a)):
        for idx_b in range(len_b):
            ...
            # min_sq_dist = np.inf
            # dist_vec = None
            # for i in tqdm(range(points_a[idx_a].shape[0])):
            #     for j in range(points_b[idx_b].shape[0]):
            #         diff = points_a[idx_a][i] - points_b[idx_b][j]
            #         dist = np.dot(diff, diff)
            #         if dist < min_sq_dist:
            #             min_sq_dist = dist
            #             dist_vec = diff * diff

            diff = points_a[idx_a][:, np.newaxis, :] - points_b[idx_b][np.newaxis, :, :]
            distances = np.sum(diff**2, axis=2)
            min_sq_dist_idx = np.unravel_index(np.argmin(distances), distances.shape)
            dist_vec = diff[min_sq_dist_idx] * diff[min_sq_dist_idx]
